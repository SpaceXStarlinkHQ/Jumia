...
// Pulls the first token after "—" that looks like a model number (letters+digits,
// no spaces). Skips pure dimension / weight tokens like "50kg" or "6×4.5ft".
function extractModel(name: string): string {
  const parts = name.split("—");
  for (let i = 1; i < parts.length; i++) {
    const token = parts[i].trim().split(/\s+/)[0];
    if (!token) continue;
    // Skip size/weight tokens that start with a digit
    if (/^\d/.test(token)) continue;
    // Must contain at least one letter and one digit (or be all-caps letters) to be a SKU
    if (/[A-Za-z]/.test(token) && (/\d/.test(token) || /^[A-Z]{2,}$/.test(token))) {
      return token.replace(/[^A-Za-z0-9\-\/]$/, ""); // strip trailing punctuation
    }
  }
  return "";
}
...
// ── CDN → brand alignment ──────────────────────────────────────────────────────
// Maps image proxy hosts to the brand(s) whose products should use them.
// A mismatch means the primary image CDN doesn't belong to this product's brand.
const CDN_BRAND_MAP: Record<string, string[]> = {
  "firmanpowerequipment.com": ["Firman", "Sumec"],
  "www.danby.com":            ["Haier", "Thermocool", "Danby"],
  "store.storeimages.cdn-apple.com": ["Apple"],
  "resource.logitech.com":    ["Logitech"],
  "static.nike.com":          ["Nike"],
  "techmall-images-repo.s3.eu-west-2.amazonaws.com": ["LG", "Samsung", "Hisense", "Tecno", "Infinix"],
};

function checkCdnBrandAlignment(imageUrl: string, productName: string): string | null {
  try {
    const hostname = new URL(imageUrl).hostname;
    const allowedBrands = CDN_BRAND_MAP[hostname];
    if (!allowedBrands) return null; // Unsplash or unmapped CDN — skip
    const match = allowedBrands.some(b => productName.toLowerCase().includes(b.toLowerCase()));
    if (!match) {
      return `Primary image is from ${hostname} (expected brands: ${allowedBrands.join(", ")}) but product name doesn't contain any of those brands.`;
    }
  } catch {
...
async function validateCatalog() {
  ...
  for (const product of products) {
    ...
    // ── 2b. CDN brand alignment ──────────────────────────────────────────────
    if (images.length > 0) {
      const cdnIssue = checkCdnBrandAlignment(images[0], product.name);
      if (cdnIssue) issues.push({ level: "warn", message: cdnIssue });
    }

    // ── 2c. Model number in description ──────────────────────────────────────
    const model = extractModel(product.name);
    if (model && !product.description.includes(model)) {
      issues.push({
        level: "warn",
        message: `Model number "${model}" (from product name) does not appear in the description.`,
      });
    }
